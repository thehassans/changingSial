      ...
      if (normItems.length > 0) {
        for (const it of normItems) {
           ...
           if (p) {
             ...
             const costPrice = p.dropshippingPrice != null ? p.dropshippingPrice : sellingPrice; 
             // If dropshippingPrice is 0 or not set, assume no profit or full profit?
             // Assumption: If dropshippingPrice is 0, maybe cost is 0? Or it wasn't set.
             // Let's assume profit = price - dropshippingPrice.
             totalProfit += Math.max(0, sellingPrice - costPrice) * it.quantity;
           }
        }
      } else if (prod) {
        const sellingPrice = prod.price || 0;
        const costPrice = prod.dropshippingPrice != null ? prod.dropshippingPrice : sellingPrice;
        totalProfit = Math.max(0, sellingPrice - costPrice) * Math.max(1, Number(quantity || 1));
      }
