...
router.post('/', auth, allowRoles('admin','user','manager'), upload.any(), async (req, res) => {
  const { name, price, dropshippingPrice, stockQty, purchasePrice, category, madeInCountry, description, stockUAE, stockOman, stockKSA, stockBahrain, stockIndia, stockKuwait, stockQatar } = req.body || {}
  ...
  const doc = new Product({
    ...
    price: Number(price),
    dropshippingPrice: dropshippingPrice != null ? Number(dropshippingPrice) : 0,
...
router.patch('/:id', auth, allowRoles('admin','user','manager'), upload.any(), async (req, res) => {
  ...
  const { name, price, dropshippingPrice, stockQty, purchasePrice, category, madeInCountry, description, inStock, stockUAE, stockOman, stockKSA, stockBahrain, stockIndia, stockKuwait, stockQatar } = req.body || {}
  if (name != null) prod.name = String(name).trim()
  if (price != null) prod.price = Number(price)
  if (dropshippingPrice != null) prod.dropshippingPrice = Number(dropshippingPrice)
