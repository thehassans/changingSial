...
router.post(
  ...
  async (req, res) => {
    ...
    if (!allowedCodes.some((code) => phoneClean.startsWith(code))) {
      return res.status(400).json({
        message:
          "Phone number must be from UAE, Oman, KSA, Bahrain, Pakistan, Kuwait, Qatar, or India",
      });
    }
    ...
    // Try to send WhatsApp welcome message
    (async () => {
      try {
        const digits = String(phone || "").replace(/\D/g, "");
        if (digits) {
          const jid = `${digits}@s.whatsapp.net`;
          const text = `🌟 Welcome to VITALBLAZE Commerce!\n\nDear ${firstName} ${lastName},\n\nWe’re excited to have you on board as a Dropshipper. Your account has been successfully created.\n\n🌐 Login URL: https://web.buysial.com/login\n\n👤 Email: ${email}\n🔑 Password: ${password}\n\nStart selling our premium products and earn profits today!\n\nWelcome to the future of smart commerce with VITALBLAZE!`;
          const wa = await getWA();
          try {
            await wa.sendText(jid, text);
            await User.updateOne({ _id: dropshipper._id }, { $set: { welcomeSent: true, welcomeSentAt: new Date(), welcomeError: "" } });
          } catch (e) {
            await User.updateOne({ _id: dropshipper._id }, { $set: { welcomeSent: false, welcomeError: String(e?.message||'failed').slice(0, 300) } });
          }
        } else {
            await User.updateOne({ _id: dropshipper._id }, { $set: { welcomeSent: false, welcomeError: "no-phone" } });
        }
      } catch (err) {
        console.error("[dropshippers] failed to send welcome WA", err?.message);
      }
    })();
...
// List dropshippers (admin => all, user => own)
router.get(
  "/dropshippers",
  auth,
  allowRoles("admin", "user"),
    ...
    try {
      ...
      if (phone !== undefined) {
         ...
         if (!allowedCodes.some(c=> phoneClean.startsWith(c)))
           return res.status(400).json({ message: "Phone invalid country code" });
